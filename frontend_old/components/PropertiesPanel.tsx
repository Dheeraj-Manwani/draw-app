...
export default function PropertiesPanel({
  ...
  if (!hasSelection) {
    return (
      <aside className="w-80 bg-card/95 backdrop-blur-sm border-l border-border shadow-lg p-6 overflow-y-auto h-full">
                ...
                <button
                  ...
                  className={`w-7 h-7 rounded-md border-2 transition-all duration-200 hover:scale-110 ${
                    selectedElement.strokeColor === color
                      ? "border-foreground shadow-md ring-2 ring-primary/20"
                ...
                >
                  {color === "transparent" && (
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="w-3 h-0.5 bg-muted-foreground/60 rotate-45"></div>
                    </div>
                  )}
                </button>
              ))}
