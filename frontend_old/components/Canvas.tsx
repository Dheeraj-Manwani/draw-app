...
export default function Canvas({
  ...
  const drawBackground = useCallback(() => {
    ...
    ctx.restore();
  }, [backgroundType, backgroundColor, zoom, panX, panY]);
      ...
      {textInput && (
        ...
        >
          <input
            type="text"
            autoFocus
            className="px-2 py-1 text-sm bg-transparent text-foreground focus:outline-none transition-all duration-200 resize-none"
            style={{
              fontSize: `${(textInput.element.data?.fontSize || 16) * zoom}px`,
              fontFamily:
                textInput.element.data?.fontFamily || "Inter, sans-serif",
              color: textInput.element.strokeColor,
            ...
            onInput={(e) => {
              ...
              if (canvas) {
                const ctx = canvas.getContext("2d");
                if (ctx) {
                  const fontSize = textInput.element.data?.fontSize || 16;
                  const fontFamily =
                    textInput.element.data?.fontFamily || "Inter, sans-serif";
                  ctx.font = `${fontSize}px ${fontFamily}`;
                  const textWidth = ctx.measureText(target.value).width;
                  const newWidth = Math.max(200, textWidth + 20) * zoom;
